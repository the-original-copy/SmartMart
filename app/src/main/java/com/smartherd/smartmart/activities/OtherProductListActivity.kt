...
import com.smartherd.smartmart.databinding.ActivityOtherProductListBinding
...
class OtherProductListActivity : BaseActivity() {
    lateinit var binding: ActivityOtherProductListBinding
    lateinit var averageLocation: String
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityOtherProductListBinding.inflate(layoutInflater)
        setContentView(binding.root)
        window.setFlags(
            WindowManager.LayoutParams.FLAG_FULLSCREEN,
            WindowManager.LayoutParams.FLAG_FULLSCREEN
        )
        setActionBar(binding.toolbarOtherProductListActivity,"w")
        if(intent.hasExtra(Constants.AVERAGE_LOCATION)){
            ...
            FireBaseClass().getProductList(this,Constants.PRODUCT_CATEGORY,Constants.PRODUCTS)
        }
    }

    fun assignThisOtherProductList(list: ArrayList<Product>) {
        populateProductLists(list,binding.rvOtherProductList,binding.tvNoOtherProductCreated,this)
    }
